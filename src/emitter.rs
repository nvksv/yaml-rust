...
pub fn yaml_dump<'a>(writer: &'a mut fmt::Write, doc: &Yaml) -> EmitResult {
    let mut emitter = YamlEmitter::new(writer);
    emitter.dump(doc)
}

pub fn yaml_dump_compact<'a>(writer: &'a mut fmt::Write, doc: &Yaml, compact: bool) -> EmitResult {
    let mut emitter = YamlEmitter::new(writer);
...
bool1: false"#;
        ...
        let doc = yaml_load_doc_from_str(&input).unwrap();
        let mut writer = String::new();
        yaml_dump(&mut writer, &doc).unwrap();
...
e:
  ...
  -
    h: []"#
        ...
        let mut writer = String::new();
        yaml_dump_compact(&mut writer, &doc, compact).unwrap();
